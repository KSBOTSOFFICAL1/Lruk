...
async def handle_record(client: Client, message: Message):
    ...
    # IMPORTANT: Resolve built-in channel names such as "Pogo" before parsing.
    # The /rec handler already does this for its flag parser, but the old
    # handle_record() parsed message.command directly, so:
    #   /rec Pogo 00:01:00
    # incorrectly treated "Pogo" as the stream URL.
    resolved_text = _resolve_channel_prefix(message.text or "")
    url, timestamp, raw_filename, flags = _shlex_parse_rec(resolved_text)

    if not url or not timestamp:
        return await message.reply_text(
            "Bad arguments. Use `/rec <channel|link> HH:MM:SS [filename]`."
        )
    ...
    if dur_sec <= 0:
        return await message.reply_text(
            "❌ Invalid duration. Use `HH:MM:SS`, for example `00:01:00`."
        )
    ...
    setup: dict = {
        ...
        # Preserve inline flags when handle_record() is called directly.
...
# Maximum number of simultaneous processing jobs allowed by the shared
# video-processing workflow.  This is deliberately independent of
# user_tasks, because user_tasks is legacy single-job state used by some
# older commands.
MAX_ACTIVE_PROCESSING_JOBS = 10

def _active_processing_job_count() -> int:
    """Return active unified jobs plus pending compression menus."""
    running = sum(1 for job in _processing_jobs.values() if not job.get("terminal"))
    pending_compress = len(compress_jobs)
    return running + pending_compress

def _processing_capacity_available() -> bool:
    return _active_processing_job_count() < MAX_ACTIVE_PROCESSING_JOBS

def _processing_capacity_message() -> str:
    return (
        f"⚠️ Maximum `{MAX_ACTIVE_PROCESSING_JOBS}` processing jobs are already running.\n\n"
...
async def rec_cmd(client: Client, message: Message):
    ...
    if len(message.command) < 3:
        return await message.reply_text(
            "**Usage:** `/rec <channel|link> HH:MM:SS [filename]`\n\n"
...
async def compress_cmd(client: Client, message: Message):
    ...
    if user_id in compress_jobs:
        return await message.reply_text("You already have an active compress job. Use /statusme or /cancelme.")
    if not _processing_capacity_available():
        return await message.reply_text(_processing_capacity_message())
...
@app.on_callback_query(filters.regex(r"^ss:(?:n:(?:[1-9]|[12][0-9]|30)|cancel)$"))
async def ss_callback(client: Client, cq: CallbackQuery):
    """Handle the /Screenshot 1-30 + Cancel menu.

    The previous callback imported SS_MIN/SS_MAX from a separate ``logic``
    module.  This bot already defines the screenshot limits locally, so that
    import could raise ModuleNotFoundError and leave the button callback
    broken.  Keep the callback self-contained and always acknowledge it.
    """
    user_id = cq.from_user.id
    data = cq.data or ""
    LOG.info("Screenshot callback received: %s", data)

    # Always stop Telegram's callback spinner immediately.
    try:
    ...
    if not state:
        try:
            await cq.answer("This screenshot menu is no longer active.", show_alert=True, cache_time=0)
        except Exception:
            pass
        return
    ...
    if action == "cancel":
        ...
        try:
            await cq.message.edit_text("❌ Screenshot selection cancelled.", reply_markup=None)
        except Exception:
            pass
        return
    ...
    if action == "n" and len(parts) == 3:
        try:
            ...
            try:
                await cq.answer("Invalid screenshot number.", show_alert=True, cache_time=0)
            except Exception:
                pass
            return

        # Use the constants defined in this same bot.py.  Do NOT import a
        # separate ``logic`` module here.
        if not (SS_MIN <= n <= SS_MAX):
            try:
                await cq.answer(f"Choose between {SS_MIN} and {SS_MAX}.", show_alert=True, cache_time=0)
            except Exception:
                pass
            return

        # Remove the menu session before starting the job so a second click
        # cannot start another screenshot job for the same user.
        ss_jobs.pop(user_id, None)
        try:
            await cq.message.edit_text(
                f"📸 Generating **{n}** screenshot{'s' if n != 1 else ''}...",
                reply_markup=None,
            )
        except Exception:
            pass
    ...
    try:
        await cq.answer("Invalid screenshot button.", show_alert=True, cache_time=0)
    except Exception:
        pass
...
async def wmp_callback(client: Client, cq: CallbackQuery):
    ...
    # ── Start pressed ──────────────────────────────────────────────────────
    if not _processing_capacity_available():
        return await cq.answer(_processing_capacity_message(), show_alert=True)
